...
void CAD_Init(uint8_t PortNum, USBPD_SettingsTypeDef *pSettings, USBPD_ParamsTypeDef *pParams,  void (*WakeUp)(void))
{
  ...
  /* The H7R3 UCPD kernel clock is fixed at hsi_ck / 4 = 16 MHz (there is no
     RCC mux for it).  With PSC = /2 the UCPD peripheral clock (ucpd_clk) is
     8 MHz, inside the 6-9 MHz range the IP is specified for.
     CFG1.HBITCLKDIV divides ucpd_clk (already pre-scaled by PSC) and yields
     hbit_clk = ucpd_clk / (HBITCLKDIV + 1).  For the USB-PD nominal 300 kbit/s
     BMC bit rate the half-bit period is ~1.67 us, so at 8 MHz:
       HBITCLKDIV = 0x0D  -> 14 clocks/half-bit  -> bit rate ~285.7 kbit/s
       TRANSWIN   = 0x07  -> 8 half-bits          -> tTransitionWindow ~14 us
       IFRGAP     = 0x10  -> 17 clocks            -> tInterFrameGap ~2.1 us
     These are the LL_UCPD_StructInit defaults.  Do NOT use the 0x1A/0x09/0x0E
     triple copied from an STM32U5 forum post: HBITCLKDIV = 0x1A divides the
     8 MHz pre-scaled clock by 27 and produces a ~148 kbit/s half-bit clock,
     far outside the 270-330 kbit/s window, so every PD frame is rejected. */
  settings.psc_ucpdclk = LL_UCPD_PSC_DIV2;
  settings.transwin = 0x07U;
  settings.IfrGap = 0x10U;
  settings.HbitClockDiv = 0x0DU;
  (void)LL_UCPD_Init(Ports[PortNum].husbpd, &settings);
  /* The CC comparator can still detect Rp without this, but the PD BMC
     receiver is unreliable/noisy when the UCPD analog RX filter is left at
     its reset setting.  Enable it before listening for PD packets. */
  LL_UCPD_RxAnalogFilterEnable(Ports[PortNum].husbpd);
