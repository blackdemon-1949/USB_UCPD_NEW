...
#include "pdport_app.h"   /* pdsink C seam: 1 ms pump (PDEngine/port) */
#endif
...
int main(void)
{
  ...
  APP_DIAG_Init();
#endif
  /* APP_EPR_Init() only zeroes APP_EPR_Ctx and applies the ceiling
   * defaults, so it runs in every profile (the pdsink profile forces
   * APP_ENG_EPR off but the epr command and the AVS clamp still use
   * APP_EPR_Ctx.ceiling_mv). */
  APP_EPR_Init();
  ...
  while (1)
  {
    /* USER CODE END WHILE */
#if defined(PDENGINE_PDSINK)
    pdport_service();   /* pdsink 1 ms pump (replaces USBPD_DPM_Run) */
#else
    USBPD_DPM_Run();
#endif
