...
/* USER CODE BEGIN USBPD_VDM_InformModeEnter */
  /* Record the responder status for `vdm status`.  State recording only -
   * no printf or CDC from this callback. */
  APP_VDM_OnModeEnter(PortNum, (uint8_t)SOPType, (uint32_t)CommandStatus,
                      SVID, ModeIndex);
...
/* USER CODE BEGIN USBPD_VDM_InformModeExit */
  APP_VDM_OnModeExit(PortNum, (uint8_t)SOPType, (uint32_t)CommandStatus,
                     SVID, ModeIndex);
  switch(CommandStatus)
  {
  case SVDM_RESPONDER_ACK :
