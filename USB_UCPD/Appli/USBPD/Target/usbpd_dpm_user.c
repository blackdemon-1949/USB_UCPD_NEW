...
/* USER CODE BEGIN USBPD_DPM_Notification */
  ...
  APP_EPR_OnNotify((uint32_t)EventVal);
...
/* USER CODE BEGIN USBPD_DPM_HardReset */
  /* Sink-side hard-reset handling.
   *
   * This board has NO VBUS ADC (see BSP_USBPD_PWR_VBUSGetVoltage), so the PE's
   * VSAFE0V / VSAFE5V checks read the software value maintained by APP_PD and
   * optionally the INA226 on I2C2.  That value was never cleared when a hard
   * reset began, so BSP_USBPD_PWR_VBUSIsOn() kept reporting VBUS present and
   * PE_SNK_HARD_RESET_WAIT_VSAFE_0V could not observe VSAFE0V - a credible
   * cause of the repeated
   *   WAIT_FOR_CAPABILITIES -> HARD_RESET -> WAIT_VSAFE_0V -> USBSTACK_STOP/START
   * loop.  Clearing it at the start of the reset makes the synthetic value
   * match what the source is actually doing (driving VBUS to 0).
   *
   * No blocking work here: one counter increment, one comparison and one
   * integer store.  This runs on the PD path. */
  APP_DIAG_Inc(APP_DIAG_PD_HARD_RESET);

  if ((Status == USBPD_HR_STATUS_START_ACK) ||
      (Status == USBPD_HR_STATUS_START_REQ))
  {
    if (CurrentRole == USBPD_PORTPOWERROLE_SNK)
    {
      APP_PD_SetVbusMv(0u);
    }
    DPM_USER_DEBUG_TRACE(PortNum, "DPM_HardReset start: VBUS expectation cleared");
  }
  else if (Status == USBPD_HR_STATUS_FAILED)
  {
    DPM_USER_DEBUG_TRACE(PortNum, "DPM_HardReset FAILED");
  }
