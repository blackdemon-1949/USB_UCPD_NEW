...
__weak int32_t BSP_USBPD_PWR_VBUSInit(uint32_t Instance)
{
  ...
  /* This board is a CC-only PD sink.  It has NO VBUS ADC and NO VBUS source
   * control, so there is no VBUS peripheral to bring up here.  VBUS is
   * reported to the PE from software state (APP_PD_GetVbusMv) and, only when
   * the optional INA226 on I2C2 is fitted and the user selected it with
   * 'ina vbus real', from the measured rail.
   *
   * The previous body returned BSP_ERROR_NONE for work it never performed and
   * logged "ADVICE", which read as a fault in CubeMonitor.  Instead, verify
   * the thing that genuinely backs VBUS sensing on this hardware and report
   * truthfully: if the operator asked for the real INA226 rail but no INA226
   * is present, that is a genuine failure and must not be masked as success. */
  int32_t ret;
  ...
  else if ((INA226_VbusModeIsReal() != 0U) && (INA226_IsPresent() == 0U))
  {
    /* Real-VBUS mode was requested but no INA226 answered on I2C2. */
    PWR_DEBUG_TRACE(Instance, "VBUSInit FAIL: real VBUS requested, no INA226");
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
  else
  {
    /* Nothing to initialise: no VBUS ADC, no VBUS switch.  The read path in
     * BSP_USBPD_PWR_VBUSGetVoltage / VBUSIsOn needs no setup of its own. */
    PWR_DEBUG_TRACE(Instance, "VBUSInit: CC-only sink, no VBUS ADC (synthetic/INA226 read path)");
    ret = BSP_ERROR_NONE;
  }
