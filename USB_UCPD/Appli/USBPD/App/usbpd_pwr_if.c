...
#if USBPD_CC_ONLY_TESTER
  /* There is deliberately no VBUS wire/ADC on this test fixture.  The UCPD
   * CAD state machine uses these predicates only as policy gates:
   * VSAFE5V is required before it announces attach, and SNKDETACH must be
   * true so that the CC state (not a nonexistent VBUS ADC) detects unplug.
   * The CC comparator remains the authority for attach/detach. */
  UNUSED(PortNum);
