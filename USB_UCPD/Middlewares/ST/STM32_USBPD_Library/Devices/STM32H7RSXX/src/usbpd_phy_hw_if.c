...
/* UCPD GPDMA and Cortex-M7 D-cache are not coherent. The PD library's
 * protocol RX buffer is supplied by the closed stack, so maintain cache at
 * each DMA boundary instead of relying on an application linker section. */
static void USBPD_CacheClean(const void *address, uint32_t size)
{
  uintptr_t start = ((uintptr_t)address) & ~(uintptr_t)31U;
  uint32_t length = (uint32_t)(((uintptr_t)address + size - start + 31U) & ~(uintptr_t)31U);
  SCB_CleanDCache_by_Addr((uint32_t *)start, (int32_t)length);
  __DSB();
}

static void USBPD_CacheInvalidate(const void *address, uint32_t size)
{
  uintptr_t start = ((uintptr_t)address) & ~(uintptr_t)31U;
  uint32_t length = (uint32_t)(((uintptr_t)address + size - start + 31U) & ~(uintptr_t)31U);
...
void HW_SignalAttachement(uint8_t PortNum, CCxPin_TypeDef cc)
{
  ...
  USBPD_CacheInvalidate(Ports[PortNum].ptr_RxBuff, SIZE_MAX_PD_TRANSACTION_UNCHUNK);
