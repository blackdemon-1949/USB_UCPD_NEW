...
/**
  * @brief  Diagnostic counters incremented by the UCPD1 IRQ handler.
  *         Exposed so the application CLI can dump the PD PHY activity.
  */
typedef struct
{
  volatile uint32_t ucpd_irq;      /*!< Total UCPD1 IRQ entries                          */
  volatile uint32_t typecevt;      /*!< Type-C (CC) events                               */
  volatile uint32_t rxorddet;      /*!< RX ordered set (SOP/hard reset) detected         */
  volatile uint32_t rxmsgend_ok;   /*!< RX message completed without error               */
  volatile uint32_t rxmsgend_err;  /*!< RX message completed with error (CRC/overrun)    */
  volatile uint32_t rxovr;         /*!< RX overrun events                                */
  volatile uint32_t rxhrstdet;     /*!< RX hard reset detected                           */
  volatile uint32_t txmsgsent;     /*!< TX message fully transmitted                     */
  volatile uint32_t txmsgdisc;     /*!< TX message discarded                             */
  volatile uint32_t txhrstsent;    /*!< TX hard reset transmitted                        */
  volatile uint32_t txmsgabt;      /*!< TX message aborted                               */
