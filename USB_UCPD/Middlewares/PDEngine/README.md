...
- [x] M4 ST transport `port/pd_tr_st.c` (syntax-gated against the real
      ...
      **M4-app re-pointing is complete and committed**: every app call
      site now carries the `#if defined(PDENGINE_PDSINK)` branch
      (`app_epr.c` EPR verbs/status on the seam snapshot, `app_pd.c`
      request engine through `pdport_*`, `app_vdm_target.c` /
      `app_cable_target.c` truthful reject/no-cable, `app_pdcap.c` and
      `Core/src/usbpd_trace.c` closed-core registration guarded,
      `main.c` pump + `usbpd.c` init).  Default build (define unset) is
      byte-identical to the closed-core path; VDM/structured-Get tester
      commands have no pdsink engine and report n/a on that path.
      Remaining is board-side only: build with `PDENGINE_PDSINK` defined
      and flash (steps in `port/README.md`; shared with the M5 bench).
- [x] M5 EPR bench milestone (host): pdsink PE auto-entry via an
      ...
      stalled-caps hard-reset recovery, plus a PD 3.2-flavoured source
      (SPR AVS APDO in the caps) that still completes the full EPR
      conversation — 11 stack tests + the M4-app glue scenarios.
      **Board acceptance remains** (no live board here):
...
PD revision coverage (PD 2.0 / 3.0 / 3.1 / 3.2): the 2-bit header
"Spec Revision" field carries only 01b (PD 2.0) and 10b (the whole
PD 3.x family); USB PD 3.1 and 3.2 have no separate header value and
signal themselves through PDO/RDO content (EPR Mode Capable bit, EPR
PDOs at positions 8+, SPR AVS APDOs) — see the mapping documented in
`pdsink/src/pd/data_objects.h`.  The pdsink core implements the
PD 3.1 EPR conversation (enter/exit/keep-alive/chunked EPR caps/AVS
requests) and accepts PD 3.2 SPR AVS source objects; the CLI prints
the negotiated revision with the family label ("PD 3.x (10b:
3.0/3.1/3.2)") and reports EPR capability separately from the source
caps, so a PD 3.1 charger is never mislabelled.
