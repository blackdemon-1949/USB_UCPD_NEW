...
static void ina_sample(void)
{
  ...
  s.bus_errs = 0U;
  /* Feed the energy accumulator (mAh/mWh integration + SD checkpointing). */
  INA226_Energy_OnSample(s.bus_mv, s.cur_ua);
  return;
...
void EXT_I2C_FeatureInit(void)
{
  ...
  /* Energy accumulator (mAh / mWh) with periodic SD checkpointing. */
  INA226_Energy_Init();
...
void EXT_I2C_FeaturePoll(void)
{
  ...
  /* Energy accumulator runs every poll; internally it time-gates SD
   * checkpoints to once a minute, so the super loop is never blocked. */
  INA226_Energy_Poll();
}
