...
void APP_PD_Init(void)
{
  ...
  s_caps_printed = 0U;
  s_sweep_active = 0U;
  /* Cable-probe state intentionally omitted: see Phase 3 note in
     usbpd_dpm_conf.h — VCONN sourcing is not available on this rig. */
}
...
void APP_PD_OnNotify(uint8_t port, USBPD_NotifyEventValue_TypeDef ev)
{
  ...
  switch (ev)
  {
    case USBPD_NOTIFY_POWER_EXPLICIT_CONTRACT:
      ...
      /* Phase 3 A1 note: SOP' cable discovery requires VCONN sourcing, which
         this board does not implement (no VCONN power FET, one CC line only
         per README).  We no longer auto-schedule SOP' Discover Identity.
         The 'cable' CLI command remains available for manual testing / future
         hardware where VCONN is wired in. */
      break;
