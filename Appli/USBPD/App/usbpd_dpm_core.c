...
USBPD_StatusTypeDef USBPD_DPM_InitCore(void)
{
  ...
  static const USBPD_PE_Callbacks dpmCallbacks =
  {
    ...
    NULL  /* USBPD_PE_RequestDPMWhatToDo: EPR-gated; left unset so EPR mode is
             not entered.  EPR power is disabled on this board. */
  };
  ...
  /* Initialise the TRACE (TRACER_EMB over USART3 -> STM32CubeMonitor-UCPD) */
  USBPD_TRACE_Init();
...
void USBPD_DPM_Run(void)
{
   ...
   * function after USBPD_DPM_Run(), including USB CDC RX processing, CDC TX
   * flushing and the LED task.  Run one cooperative slice instead.
   */
