...
/* USER CODE BEGIN USBPD_DPM_EvaluateVconnSwap */
  /* Phase 3: accept VCONN swap so we can source VCONN and issue SOP' Discover
     Identity to read the cable e-marker. */
...
/* USER CODE BEGIN USBPD_DPM_PE_VconnPwr */
  /* Phase 3 A1 correction: this board has no VCONN power-switch FET and
     BSP_USBPD_PWR_VCONNOn returns FEATURE_NOT_SUPPORTED.  Returning USBPD_OK
     here would lie to the PE and cause the state machine to attempt SOP'
     Discover Identity with no VCONN actually present on the idle CC line,
     which reliably times out and pollutes the trace.  Leave the default
     USBPD_ERROR so the PE knows VCONN cannot be sourced. */
  (void)PortNum;
  (void)State;
  return USBPD_ERROR;
...
/* USER CODE BEGIN USBPD_DPM_RequestVDM_DiscoveryIdentify */
  /* Phase 3: allow Discover Identity on SOP' (cable plug) and SOP'' (active
     cable far-end) as well as SOP (port partner), so callers can probe the
     e-marker for current / Vmax / EPR ratings. */
  switch (SOPType)
  {
