...
/* USER CODE BEGIN USBPD_VDM_InformIdentity */
  /* Phase 3: dispatch on SOPType. SOP replies describe the port-partner
     (source); SOP' / SOP'' replies describe the cable plug, and they go to
     APIE_Cable_OnIdentity so their VDO fields (current cap / Vmax / EPR) are
     attributed to the cable, not the source. */
  if (CommandStatus != SVDM_RESPONDER_ACK)
  {
    if (SOPType == USBPD_SOPTYPE_SOP)
    {
      APP_PD_PrintIdentity(NULL, 0U);
    }
    else
    {
      APP_LOG_Printf("[PD] %s Discover Identity NAK (non-e-marked cable or VCONN swap rejected).\r\n",
                     APIE_SopName(SOPType));
    }
  ...
  else if ((SOPType == USBPD_SOPTYPE_SOP1) || (SOPType == USBPD_SOPTYPE_SOP2))
  {
    /* Cable plug reply — extract VDO presence flags and pass raw d32 values to
       the cable tracker (it decodes fields with explicit bit shifts). */
    uint8_t passive = (uint8_t)pIdentity->CableVDO_Presence;
    uint8_t active  = (uint8_t)pIdentity->ActiveCableVDO1_Presence;
    uint32_t cable_d = passive ? pIdentity->CableVDO.d32 : 0U;
    uint32_t act_d   = active  ? pIdentity->ActiveCableVDO1.d32 : 0U;
    APIE_Cable_OnIdentity((uint8_t)SOPType,
                          pIdentity->IDHeader.d32,
                          pIdentity->CertStatVDO.d32,
