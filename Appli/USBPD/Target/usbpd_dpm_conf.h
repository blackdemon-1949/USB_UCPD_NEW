...
{
  {
    /* Phase 3 NOTE on SOP'/SOP'':
       SOP'/SOP'' addressing is enabled in PE_SupportedSOP so the protocol
       stack will accept and route cable-plug VDMs if they ever arrive, but
       VCONN sourcing is NOT enabled on this hardware (no VCONN power FET,
       BSP_USBPD_PWR_VCONNOn returns FEATURE_NOT_SUPPORTED; see README.md
       "one CC line to the MCU header"). As a pure SINK without VCONN source
       capability, this port CANNOT originate SOP' Discover Identity on its
       own — only the DFP/source end of the cable (which does source VCONN)
       can do that, and the SOP' reply does not get forwarded back to the
       Sink. Cable VDO fields therefore stay "unknown" until either
        (a) VCONN hardware is added and PE_VconnSwap/USBPD_DPM_PE_VconnPwr
            are implemented in the BSP, or
        (b) the port operates in DRP/source mode (out of scope here).
       The apie_cable decoder remains wired up so that, in a future hardware
       revision where VCONN is sourced, parsing works with no further code
       changes, and any VDM_InformIdentity callback with SOPType != SOP
       (e.g. from PD 3.1 EPR loops where the source relays data) is routed
...
{
  {
    ...
    .PE_VconnSwap = USBPD_FALSE,                /* VCONN swap rejected: no VCONN power FET on this board (Phase 3 note above) */
